...
function parseArgs(args: string[]) {
    ...
    if (!clientId) {
        ...
        console.error("  --color <hex>            Primary color (default: #000000)");
        console.error(
            "  --position <pos>         Widget position: left or right (default: right)",
        );
        console.error("  --disabled               Create settings with widget disabled");
        process.exit(1);
    }
    ...
    if (colorIndex !== -1 && args[colorIndex + 1]) {
        ...
        if (!/^#[0-9a-fA-F]{6}$/.test(primaryColor)) {
            console.error("❌ Error: Invalid color format. Expected hex color (e.g., #FF0000)");
            process.exit(1);
        }
    }
...
async function main() {
    ...
    if (!connectionString) {
        ...
        console.error("Make sure to run with: pnpm with-env tsx script/seed-widget-settings.ts");
        process.exit(1);
    }
    ...
    try {
        ...
        if (existingSettings) {
            console.log("⚠️  Widget settings already exist for this client. Updating...");
        ...
        console.log(`     - Product Reviews: ${config.activeWidgets.productReviews}`);
