...
import { bigint, boolean, check, foreignKey, index, integer, jsonb, numeric, pgTable, serial, text, timestamp, unique, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
...
export const account = pgTable("account", {
	...
	uniqueIndex("idx_account_provider").using("btree", table.providerId.asc().nullsLast().op("text_ops"), table.accountId.asc().nullsLast().op("text_ops")),
...
export const clients = pgTable("clients", {
	...
	index("idx_clients_api_key").using("btree", table.apiKey.asc().nullsLast().op("uuid_ops")),
	index("idx_clients_user").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
		columns: [table.userId],
		foreignColumns: [user.id],
	...
	check("clients_type_check", sql`type = ANY (ARRAY['free'::text, 'hobby'::text, 'super'::text, 'agency'::text, 'pro'::text])`),
]);
...
export const clientConfig = pgTable("client_config", {
	...
	theme: jsonb().default({ "color": "#000000", "buttonIcon": null, "buttonTextColor": "#000000", "buttonBackgroundColor": "#ffffff" }),
...
export const customers = pgTable("customers", {
	...
	index("idx_customers_client").using("btree", table.clientId.asc().nullsLast().op("uuid_ops")),
	index("idx_customers_email_client").using("btree", table.email.asc().nullsLast().op("uuid_ops"), table.clientId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
		columns: [table.clientId],
		foreignColumns: [clients.id],
...
export const activity = pgTable("activity", {
	...
	index("idx_activity_client_created").using("btree", table.clientId.asc().nullsLast().op("uuid_ops"), table.createdAt.desc().nullsFirst().op("uuid_ops")),
	index("idx_activity_customer").using("btree", table.customerId.asc().nullsLast().op("uuid_ops")),
	index("idx_activity_email_client").using("btree", table.email.asc().nullsLast().op("text_ops"), table.clientId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
		columns: [table.clientId],
		foreignColumns: [clients.id],
...
export const referrals = pgTable("referrals", {
	...
	index("idx_referrals_client_active").using("btree", table.clientId.asc().nullsLast().op("uuid_ops")).where(sql`(is_active = true)`),
	index("idx_referrals_code").using("btree", table.code.asc().nullsLast().op("text_ops")),
	index("idx_referrals_customer").using("btree", table.customerId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
		columns: [table.clientId],
		foreignColumns: [clients.id],
...
export const orders = pgTable("orders", {
	...
	index("idx_orders_client_status").using("btree", table.clientId.asc().nullsLast().op("text_ops"), table.status.asc().nullsLast().op("uuid_ops")),
	index("idx_orders_email_client").using("btree", table.email.asc().nullsLast().op("uuid_ops"), table.clientId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
		columns: [table.clientId],
		foreignColumns: [clients.id],
...
export const products = pgTable("products", {
	...
	index("idx_products_client").using("btree", table.clientId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
		columns: [table.clientId],
		foreignColumns: [clients.id],
		name: "products_client_id_fkey"
	}).onDelete("cascade"),
	unique("products_client_id_product_id_key").on(table.clientId, table.productId),
...
export const reviews = pgTable("reviews", {
	...
	index("idx_reviews_client").using("btree", table.clientId.asc().nullsLast().op("uuid_ops")),
	index("idx_reviews_email_client").using("btree", table.email.asc().nullsLast().op("uuid_ops"), table.clientId.asc().nullsLast().op("text_ops")),
	...
	check("reviews_type_check", sql`type = ANY (ARRAY['product'::text, 'website'::text])`),
	check("reviews_rating_check", sql`(rating >= (0)::numeric) AND (rating <= (5)::numeric)`),
]);
...
export const questions = pgTable("questions", {
	...
	index("idx_questions_client").using("btree", table.clientId.asc().nullsLast().op("uuid_ops")),
	index("idx_questions_product").using("btree", table.productId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
		columns: [table.clientId],
		foreignColumns: [clients.id],
...
export const coupons = pgTable("coupons", {
	...
	index("idx_coupons_customer").using("btree", table.customerId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
		columns: [table.customerId],
		foreignColumns: [customers.id],
...
export const auditLogs = pgTable("audit_logs", {
	...
	index("idx_audit_client").using("btree", table.clientId.asc().nullsLast().op("uuid_ops")),
...
export const supportTickets = pgTable("support_tickets", {
	...
	index("idx_tickets_status").using("btree", table.status.asc().nullsLast().op("text_ops")).where(sql`(status <> ALL (ARRAY['resolved'::text, 'closed'::text]))`),
	foreignKey({
		columns: [table.clientId],
		foreignColumns: [clients.id],
	...
	check("support_tickets_category_check", sql`category = ANY (ARRAY['billing'::text, 'technical'::text, 'feature_request'::text, 'bug_report'::text, 'general'::text])`),
	check("support_tickets_status_check", sql`status = ANY (ARRAY['new'::text, 'open'::text, 'pending_customer'::text, 'pending_support'::text, 'resolved'::text, 'closed'::text])`),
