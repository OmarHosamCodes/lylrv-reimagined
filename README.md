...
- Database: Drizzle ORM + PostgreSQL ([packages/db](packages/db))
...
> [!NOTE]
> The [packages/db](packages/db) package targets PostgreSQL. If using another database, update [packages/db/src/schema.ts](packages/db/src/schema.ts), [packages/db/src/client.ts](packages/db/src/client.ts), and [packages/db/drizzle.config.ts](packages/db/drizzle.config.ts).
...
Deploy your Next.js app to your preferred platform:

1. Create a project and set the root directory to `apps/saas`
