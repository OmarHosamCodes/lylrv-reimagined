...
> In this template, we use `@lylrv` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@lylrv` to something like `@my-company` or `@project-name`.
...
pnpm --filter @lylrv/auth generate
```
...
3. Outputs a Drizzle-compatible schema file to the `@lylrv/db` package
