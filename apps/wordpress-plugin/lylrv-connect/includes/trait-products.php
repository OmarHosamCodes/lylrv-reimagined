...
trait Lylrv_Connect_Products
{
    ...
    public static function init_products()
    {
        ...
        add_action("template_redirect", [__CLASS__, "handle_product_template_redirect"]);
        add_filter("template_include", [__CLASS__, "maybe_override_product_template"]);

        // Register REST endpoint for block editor to search/list products.
        add_action("rest_api_init", [__CLASS__, "register_product_rest_routes"]);
    }
    ...
    public static function handle_product_template_redirect()
    {
        ...
        global $post;
        if (!$post instanceof WP_Post || self::PRODUCT_POST_TYPE !== $post->post_type) {
            return;
        }
    ...
    public static function maybe_override_product_template($template)
    {
        ...
        $plugin_template = LYLRV_CONNECT_PLUGIN_DIR . "templates/single-product.php";
    ...
    public static function fetch_product_by_slug($slug)
    {
        ...
        $url = $saas_url . "/api/widget/products/" . urlencode($slug) . "?" . http_build_query([
            "apiKey" => $api_key,
        ]);
        ...
        if (is_wp_error($response)) {
            error_log("Lylrv Connect: Product by slug fetch failed - " . $response->get_error_message());
            return null;
        }
        ...
        if (200 !== $code) {
            ...
            error_log("Lylrv Connect: Product by slug HTTP $code - URL: $url - Body: $body");
            return null;
        }
    ...
    public static function fetch_products($args = [])
    {
        ...
        $url = $saas_url . "/api/widget/products?" . http_build_query($query_params);
        ...
        if (is_wp_error($response)) {
            error_log("Lylrv Connect: Product fetch failed - " . $response->get_error_message());
            return [];
        }
        ...
        if (200 !== $code) {
            $body = wp_remote_retrieve_body($response);
            error_log("Lylrv Connect: Product fetch HTTP $code - URL: $url - Body: $body");
            return [];
        }
        ...
        if (!isset($data["products"]) || !is_array($data["products"])) {
            error_log("Lylrv Connect: Unexpected product response structure - Body: $body");
            return [];
        }
    ...
    public static function register_product_blocks()
    {
        ...
        register_block_type(
            ...
            [
                ...
                "render_callback" => [__CLASS__, "render_product_display_block"],
            ],
        );
    }
    ...
    {
        $slug = isset($attributes["productSlug"]) ? sanitize_title($attributes["productSlug"]) : "";
        ...
        if (empty($slug)) {
            ...
            if (defined("REST_REQUEST") && REST_REQUEST) {
                return '<div class="lylrv-product-display lylrv-product-display--empty">'
                    . '<p>' . esc_html__("Select a product to display.", "lylrv-connect") . '</p>'
                    . '</div>';
            }
            return "";
        }
        ...
        return self::render_product_html($product, $layout, $show_price, $show_description, $show_image);
    }
     ...
     */
    private static function render_product_html($product, $layout, $show_price, $show_description, $show_image)
    {
        ...
        $css_class = "lylrv-product-display lylrv-product-display--" . esc_attr($layout);
    ...
    public static function rest_get_products($request)
    {
        ...
        if (empty($api_key) || empty($saas_url)) {
            return rest_ensure_response([
                "products" => [],
                "error"    => "Plugin not configured. Please set the API key and SaaS URL in Lylrv Connect settings.",
            ]);
        }
        ...
        if (!empty($search)) {
            ...
            $products = array_filter($products, function ($product) use ($search_lower) {
                ...
                return false !== strpos($name, $search_lower) || false !== strpos($slug, $search_lower);
            });
