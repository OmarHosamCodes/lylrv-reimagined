...
const DEFAULT_SETTINGS = {
    ...
    appearance: { primaryColor: "#000000", position: "right" as "left" | "right" },
};

type WidgetSettingsState = {
    isEnabled: boolean;
    activeWidgets: { loyalty: boolean; reviews: boolean; productReviews: boolean };
...
function WidgetSettingsForm({ clientId }: { clientId: string }) {
    ...
    const [localSettings, setLocalSettings] = useState<WidgetSettingsState | null>(null);
    ...
    const currentSettings = isDirty && localSettings ? localSettings : normalizedSettings;
    ...
    const handleToggleWidget = (widget: "loyalty" | "reviews" | "productReviews") => {
        updateLocalSettings({
            activeWidgets: {
            ...
            <div className="rounded-lg border p-6">
                <div className="flex items-center justify-between">
                    ...
                    <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${currentSettings.isEnabled ? "bg-primary" : "bg-gray-300"
                            }`}
                    >
                        <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${currentSettings.isEnabled ? "translate-x-6" : "translate-x-1"
                                }`}
                        />
                    </button>
                </div>
            ...
            <div className="rounded-lg border p-6">
                ...
                <div className="space-y-4">
                    {(["loyalty", "reviews", "productReviews"] as const).map((widget) => (
                        ...
                        >
                            <div>
                                <h3 className="font-medium capitalize">{widget === "productReviews" ? "Product Reviews" : widget} Widget</h3>
                                <p className="text-sm text-gray-600">
                                    {widget === "loyalty" && "Show loyalty points and rewards"}
                                    {widget === "reviews" && "Display store reviews widget"}
                                    {widget === "productReviews" && "Show reviews on product pages"}
                                </p>
                            </div>
                            <button
                                type="button"
                                onClick={() => handleToggleWidget(widget)}
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${currentSettings.activeWidgets[widget]
                                        ? "bg-primary"
                                        : "bg-gray-300"
                                    }`}
                            >
                                <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${currentSettings.activeWidgets[widget]
                                            ? "translate-x-6"
                                            : "translate-x-1"
                                        }`}
                                />
                        ...
                        <div className="flex gap-2">
                            {(["left", "right"] as const).map((pos) => (
                                <button
                                    ...
                                    className={`flex-1 rounded-lg border px-4 py-2 text-sm font-medium capitalize transition-colors ${currentSettings.appearance.position === pos
