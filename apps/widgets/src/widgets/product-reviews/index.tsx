...
const SAMPLE_REVIEWS: Review[] = [
	{
		...
		content:
			"This product exceeded my expectations. The quality is top-notch and it arrived quickly.",
		author: "Emily R.",
		date: "2 days ago",
		verified: true,
	},
	{
		...
		content:
			"Very happy with my purchase. Would definitely recommend to others.",
		author: "Michael T.",
		date: "1 week ago",
		verified: true,
	},
...
function ProductReviewsApp({ config }: ProductReviewsAppProps) {
	...
	const productContext = config.context?.product;

	const averageRating =
		SAMPLE_REVIEWS.reduce((acc, r) => acc + r.rating, 0) /
					...
					<div>
						...
						<div className="mt-1 flex items-center gap-2">
							...
							<span className="text-sm text-gray-600">
								{averageRating.toFixed(1)} out of 5 ({SAMPLE_REVIEWS.length}{" "}
								reviews)
							</span>
						</div>
					</div>
				...
				<div className="border-b border-gray-200 bg-gray-50 p-4">
					...
					{user?.isLoggedIn ? (
						<div className="mb-4 flex items-center gap-2 rounded-lg bg-blue-50 px-3 py-2 text-sm text-blue-700">
							...
							<span>
								Reviewing as <strong>{user.name}</strong> ({user.email})
							</span>
							{productContext?.hasPurchased && (
